...
class ModBusConnector
{
	...
	int write_bits(const int & addr, const vector<uint8_t> & values);
	
	int write_registers(const int & addr, const vector<uint16_t> & values);
	
	int write_and_read_registers(const int & write_addr, const vector<uint16_t> & values_to_write,
